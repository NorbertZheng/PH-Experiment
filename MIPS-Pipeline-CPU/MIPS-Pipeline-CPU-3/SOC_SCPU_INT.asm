...
start:		lui	$v1, 0xf000			#r3=0xF0000000
			...
			lui	$a2, 0x0008			#r6=0x00080000
			nor	$at, $zero, $zero	#r1=0xFFFFFFFF
			slt	$v0, $at,$zero		#r2=0x00000001 (unsigned slt)
			addi	$t2, $at, -1	#r10=0xFFFFFFFE	
			sw	$a2, 4($v1)			#计数器端口:F0000004，送计数常数r6=0xf8000000
			lw	$a1, 0($v1)			#读GPIO端口F0000000状态:{counter0_out,counter1_out,counter2_out,led_out[12:0], SW} 
			add	$a1, $a1, $a1		#左移
			...
			sw	$t1, 0($a0)			#r9送r4=0xE0000000七段码端口
			lw	$t5, 20($zero)		#取存储器20单元预存数据至r13,程序计数延时常数
Loop:		lw	$a1, 0($v1)			#读GPIO端口F0000000状态:{counter0_out,counter1_out,counter2_out,led_out[12:0], SW} 
			add	$a1, $a1, $a1		#左移
			add	$a1, $a1, $a1		#左移2位将SW与LED对齐，同时D1D0置00，选择计数器通道0
			sw	$a1, 0($v1)			#r5输出到GPIO端口0xF0000000，计数器通道counter_set=00端口不变、LED=SW： {GPIOf0[21:0],LED,counter_set}
			lw	$a1, 0($v1)			#再读GPIO端口0xF0000000状态
			and	$t3,$a1,$t0			#取最高位=out0，屏蔽其余位送r11
			addi	$t5, $t5,1		#程序计数延时，本程序没有使用延迟常数或计数器查询方式，而是使用计数器中断方式
Disp:		lw	$a1, 0($v1)			#读GPIO端口F0000000状态:{counter0_out,counter1_out,counter2_out,led_out[12:0], SW} 
...
L3:			sw	$t2, 0($a0)			#SW[4:3]=00,7段显示点移位后显示
			j	Loop
...
L01:		lw	$t1, 0x20($s1)		#SW[4:3]=01，从内存取预存数字
			...
			j	Loop
Next:		sw	$a2, 4($v1)			#计数器端口:F0000004，送计数常数r6=0xf8000000
			lw	$t5, 20($zero)		#取存储器20单元预存数据至r13,程序计数延时常数
